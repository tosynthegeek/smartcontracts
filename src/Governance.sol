...
interface ICover {
    function updateUserCoverValue(address user, uint256 _coverId, uint256 _claimPaid) external;
...
contract Governance is ReentrancyGuard, Ownable2Step {
    ...
    function createProposal(ProposalParams memory params) external {
        ...
        if (!lpContract.poolActive(params.poolId)) {
        ...
        for (uint256 i = 1; i < proposalCounter; i++) {
            ...
            if (
                param.user == params.user && param.coverId == params.coverId && proposal.status != ProposalStaus.Claimed
    ...
    function vote(uint256 _proposalId, bool _vote) external {
        ...
        if (proposal.createdAt == 0) {
            revert Gov__ProposalDoesNotExist();
        }
        ...
        voters[_proposalId][msg.sender] = Voter({voted: true, vote: _vote, weight: voterWeight});
    ...
    function executeProposal() external onlyAdmin nonReentrant {
        for (uint256 i = 0; i < proposalIds.length; i++) {
            ...
            if (proposal.status == ProposalStaus.Pending && block.timestamp > proposal.deadline && !proposal.executed) {
                ...
                if (proposal.votesFor > proposal.votesAgainst) {
                    ...
                    ICoverContract.updateUserCoverValue(
                        proposal.proposalParam.user, proposal.proposalParam.coverId, proposal.proposalParam.claimAmount
                    );
    ...
    function updateProposalStatusToClaimed(uint256 proposalId) public nonReentrant {
        if (msg.sender != proposals[proposalId].proposalParam.user && msg.sender != poolContract) {
    ...
    function getProposalDetails(uint256 _proposalId) external returns (Proposal memory) {
        if (block.timestamp >= proposals[_proposalId].deadline) {
            ...
            proposals[_proposalId].timeleft = (proposals[_proposalId].deadline - block.timestamp) / 1 days;
        }
    ...
    function getAllProposals() public view returns (Proposal[] memory) {
        ...
        for (uint256 i = 0; i < proposalIds.length; i++) {
            ...
            if (block.timestamp >= result[i].deadline) {
                ...
                result[i].timeleft = (result[i].deadline - block.timestamp) / 1 days;
            }
        }
        return result;
    }
        ...
        for (uint256 i = 0; i < proposalIds.length; i++) {
            if (proposals[proposalIds[i]].deadline == 0 || proposals[proposalIds[i]].deadline >= block.timestamp) {
                ...
                if (
                    ...
                    result[index].timeleft = (result[index].deadline - block.timestamp) / 1 days;
                }
